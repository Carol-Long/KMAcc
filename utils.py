...
def compute_KME_Wrapper(X_test, y_test, model, gopt, witness_metric):
    ...
    # initialize pickkle file for storing results
    results = {}
    gammas = []
    error_wit_list = []
    yhat_proba_test_list = []
    for i, (train_index, test_index) in enumerate(kf.split(X_test)):
        ...
        gammas.append(gopt)
        error_wit_list.append(error_wit)
        yhat_proba_test_list.append(yhat_proba_test)

    results['gammas'] = gammas
    results['error_wit_list'] = error_wit_list
    results['KME_yhat_proba_test_list'] = yhat_proba_test_list

    return KME.mean(), KME_std.mean(), KME_witfit.mean(), KME_witfit_std.mean(), results
...
def mega_compute_corrected(gopt, witness_metric, X_test, y_test, f_baseline, f_kMAcc, f_LSBoost, f_MCBoost, \
                           f_base_iso, f_kMAcc_iso, f_MCBoost_iso, seed, baseline_model, prefix):

    # baseline KME, MSCE
    base_KME, base_KME_std, base_KME_witfit, base_KME_witfit_std, base_results = compute_KME_Wrapper(X_test, y_test, f_baseline, gopt, witness_metric)
    print(f"Baseline KME: {base_KME}, std {base_KME_std}")
    print(f"Baseline KME on witfit: {base_KME_witfit}, std {base_KME_witfit_std}")

    # KME for KMAcc
    kMAcc_KME, kMAcc_KME_std, kMAcc_KME_witfit, kMAcc_KME_witfit_std, KMA_results = compute_KME_Wrapper(X_test, y_test, f_kMAcc, gopt, witness_metric)
    print(f"KMAcc's KME: {kMAcc_KME}, std {kMAcc_KME_std}")
    print(f"KMAcc's KME on witfit: {kMAcc_KME_witfit}, std {kMAcc_KME_witfit_std}")

    # KME for LSBoost
    LSBoost_KME, LSBoost_KME_std, LSBoost_KME_witfit, LSBoost_KME_witfit_std, LSB_results = compute_KME_Wrapper(X_test, y_test, f_LSBoost, gopt, witness_metric)
    ...
    # KME for MCBoost
    MCBoost_KME, MCBoost_KME_std, MCBoost_KME_witfit, MCBoost_KME_witfit_std, MCB_results = compute_KME_Wrapper(X_test, y_test, f_MCBoost, gopt, witness_metric)
    ...
    base_iso_KME, base_iso_KME_std, base_iso_KME_witfit, base_iso_KME_witfit_std, base_iso_results = compute_KME_Wrapper(X_test, y_test, f_base_iso, gopt, witness_metric)
    ...
    kMAcc_iso_KME, kMAcc_iso_KME_std, kMAcc_iso_KME_witfit, kMAcc_iso_KME_witfit_std, KMA_iso_results = compute_KME_Wrapper(X_test, y_test, f_kMAcc_iso, gopt, witness_metric)
    ...
    MCBoost_iso_KME, MCBoost_iso_KME_std, MCBoost_iso_KME_witfit, MCBoost_iso_KME_witfit_std, MCB_iso_results = compute_KME_Wrapper(X_test, y_test, f_MCBoost_iso, gopt, witness_metric)
    ...
    # Add test proba to results
    results = {}
    results['All_yhat_proba_test_list'] = [base_test, kMAcc_test, LSBoost_test, MCBoost_test, base_iso_test, kMAcc_iso_test, MCBoost_iso_test]
    results['base_results'] = base_results
    results['KMA_results'] = KMA_results
    results['LSB_results'] = LSB_results
    results['MCB_results'] = MCB_results
    results['base_iso_results'] = base_iso_results
    results['KMA_iso_results'] = KMA_iso_results
    results['MCB_iso_results'] = MCB_iso_results

    # Save as pickle 
    result_path = './results/'
    filename_pkl = "{}_{}_{}.pkl".format(prefix, baseline_model, seed)
